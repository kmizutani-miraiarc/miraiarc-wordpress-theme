<?php
/**
 * Template: Archive Purchase Achievements (Fixed Page)
 * Path: /wp-content/uploads/template/page-purchase-achievements.php
 * 
 * 固定ページ用の買取実績一覧テンプレート
 * mirai-apiから買取実績データを取得して表示
 */
...
// 都道府県名を取得（API用）
if ($prefecture_param && isset($prefecture_map[$prefecture_param])) {
    ...
    <?php if ( !empty($achievements) ) : ?>
      ...
      <?php if ( $total_pages > 1 ) : ?>
        <div class="c-pagination" style="margin-top:40px;text-align:center;">
          <?php
          $pagination_base = home_url('/purchase-achievements/');
          if ($prefecture_param) {
              $pagination_base = add_query_arg('prefecture', $prefecture_param, $pagination_base);
          }
          $pagination_args = [
